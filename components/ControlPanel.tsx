    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                        <Tooltip text="Simulate a vinyl tape stop effect">
                            <button 
                                ...
                                disabled={disabled}
                                className="w-full h-20 text-sm bg-gradient-to-br from-red-900 to-red-600 rounded-lg font-bold text-white shadow-md active:scale-95 active:brightness-125 transition-all border-b-4 border-red-950 active:border-b-0 active:mt-1"
                            >
                                STOP
                            </button>
                        </Tooltip>
                ...
                <div className="bg-deep-space/80 p-6 rounded-lg ring-1 ring-white/20 shadow-lg shadow-white/5 flex flex-col">
                     <h3 className="text-2xl font-bold text-white flex items-center gap-2 mb-6">
                        <span className="text-white">◆</span> Slice Control
                        <InfoIcon text="Adjust properties for the selected slice. Change level or mute specific transients." />
                    </h3>
                    {currentSlice ? (
                        <div className="space-y-6 flex-1 flex flex-col justify-center">
                            <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-2">
                                <span className="text-lg font-bold text-hyper-cyan">Slice #{currentSlice.id}</span>
                                <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${currentSlice.isActive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                            ...
                            <Slider 
                                ...
                                onChange={(v) => selectedSliceIndex !== null && onSliceUpdate(selectedSliceIndex, { level: v })} 
                                disabled={disabled} 
                                unit="x"
                                tooltip="Adjust gain volume for this specific slice"
                                defaultValue={1.0}
                            />
                            
                             <div className="text-xs text-star-dust/50 text-center pt-4">
                                Use this slider to adjust the gain of the individual slice.
                            </div>
                        </div>
                    ) : (
                        <div className="flex-1 flex items-center justify-center text-star-dust/40 italic">
                            Select a slice to edit properties
                        </div>
                    )}
                </div>

                {/* AI Beat Generator Section */}
                <div className="bg-deep-space/80 p-6 rounded-lg ring-1 ring-hyper-cyan/50 shadow-lg shadow-hyper-cyan/10">
                     <h3 className="text-2xl font-bold text-white flex items-center gap-2 mb-6">
                    ...
                    <div className="space-y-6">
                        <div className="px-2">
                            ...
                            <Tooltip text="Adjust complexity: Left for simple house beats, Right for glitchy chaos">
                                <input 
                                    type="range" 
                                    min="0" 
                                    max="1" 
                                    ...
                                    className="w-full h-3 bg-nebula-blue rounded-lg appearance-none cursor-pointer
                                            ...
                                            [&::-webkit-slider-thumb]:border-4 [&::-webkit-slider-thumb]:border-hyper-cyan
                        ...
                        <Tooltip text="Generate a new sequence based on the complexity slider">
                            <button 
                                ...
                                disabled={disabled}
                                className="w-full py-4 bg-gradient-to-r from-hyper-cyan to-blue-600 text-white font-bold text-lg rounded-md hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-blue-500/20"
                            >
                                GENERATE PATTERN
                            </button>
                        </Tooltip>
                        ...
                        {/* Glitch Chaos Engine */}
                         <div className="pt-4 border-t border-white/10">
                            <div className="flex justify-between items-center mb-2">
                                <h4 className="text-sm font-bold text-plasma-pink uppercase">Glitch Chaos</h4>
                                <span className="text-xs text-star-dust/50">{Math.round((params.glitch?.chaos || 0) * 100)}% Prob</span>
                            </div>
                            <Slider 
                                label=""
                                min={0}
                            ...
                            <div className="flex gap-2 mt-2">
                                <button 
                                    ...
                                    className={`flex-1 py-1 text-[10px] font-bold uppercase rounded border ${params.glitch.allowReverse ? 'bg-plasma-pink/20 text-plasma-pink border-plasma-pink' : 'border-white/10 text-star-dust/40'}`}
                                >
                                    Rev
                                </button>
                                <button 
                                    onClick={() => onParamChange('glitch', { ...params.glitch, allowOctaveJump: !params.glitch.allowOctaveJump })}
                                    className={`flex-1 py-1 text-[10px] font-bold uppercase rounded border ${params.glitch.allowOctaveJump ? 'bg-plasma-pink/20 text-plasma-pink border-plasma-pink' : 'border-white/10 text-star-dust/40'}`}
                                >
                                    Octave
                                </button>
                            </div>
                ...
                <EffectSection title="Effects Rack" info="Chain of audio effects for shaping tone, dynamics, and spatial ambience.">
                     <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                         ...
                         <div className="space-y-3 p-3 bg-nebula-blue/30 rounded-md border border-white/5">
                            <h4 className="font-bold text-orange-400 text-sm uppercase tracking-wider">Vintage Tape</h4>
                        ...
                        <div className="space-y-3 p-3 bg-nebula-blue/30 rounded-md border border-white/5">
                         ...
                         {/* BitCrusher (New) */}
                        <div className="space-y-3 p-3 bg-nebula-blue/30 rounded-md border border-white/5">
                            <h4 className="font-bold text-green-400 text-sm uppercase tracking-wider">BitCrush</h4>
                        ...
                        <div className="space-y-3 p-3 bg-nebula-blue/30 rounded-md border border-white/5">
                             ...
                             <div className="flex justify-between items-center mb-1">
                                <label className="text-sm font-medium text-star-dust/80">Time</label>
                                <Tooltip text="Sync delay time to Master BPM">
                                    <button 
                                        onClick={() => onEffectParamChange('delay', 'isSynced', !params.delay.isSynced)}
                                        disabled={disabled}
                                        className={`text-[10px] font-bold px-1.5 py-0.5 rounded transition-colors border ${params.delay.isSynced ? 'bg-hyper-cyan text-deep-space border-hyper-cyan' : 'bg-transparent text-star-dust/50 border-star-dust/20'}`}
                                    >
                                        SYNC
                                    </button>
                                </Tooltip>
                             ...
                             {params.delay.isSynced ? (
                                 <select 
                                    ...
                                    disabled={disabled}
                                    className="w-full h-6 bg-nebula-blue text-xs text-white rounded border border-white/10 focus:border-hyper-cyan outline-none"
                                 >
                        ...
                        <div className="space-y-3 p-3 bg-nebula-blue/30 rounded-md border border-white/5">
                            ...
                            <div className="flex justify-between items-center mb-1">
                                <label className="text-sm font-medium text-star-dust/80">Decay</label>
                                <Tooltip text="Sync reverb decay to Master BPM">
                                    <button 
                                        onClick={() => onEffectParamChange('reverb', 'isSynced', !params.reverb.isSynced)}
                                        disabled={disabled}
                                        className={`text-[10px] font-bold px-1.5 py-0.5 rounded transition-colors border ${params.reverb.isSynced ? 'bg-hyper-cyan text-deep-space border-hyper-cyan' : 'bg-transparent text-star-dust/50 border-star-dust/20'}`}
                                    >
                                        SYNC
                                    </button>
                                </Tooltip>
